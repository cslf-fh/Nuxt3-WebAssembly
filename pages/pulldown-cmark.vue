...
import { pulldown_cmark } from '../wasm/pulldown-cmark/pkg/pulldown_cmark';

const backquote = '`';
const backquotes = '```';
const inputText = ref(`# pulldown-cmark
...
you can call ${backquote}into_offset_iter()${backquote} to create an iterator that yields ${backquote}(Event, Range)${backquote}
...
${backquote}examples${backquote} directory of this repository.
...
for loops and ${backquote}map${backquote} (as in the examples above), collecting the events into
a vector (for recording, playback, and manipulation), and more.

Further, the ${backquote}Text${backquote} event (representing text) is a small copy-on-write string.
...
target like a ${backquote}Vec<u8>${backquote} or ${backquote}String${backquote}. Since it performs many (very) small writes, writing
directly to stdout, files, or sockets is detrimental to performance. Such writers can
be wrapped in a [${backquote}BufWriter${backquote}](https://doc.rust-lang.org/std/io/struct.BufWriter.html).
...
Or put in your ${backquote}Cargo.toml${backquote} file:
...
Or add the feature to your project's ${backquote}Cargo.toml${backquote}:
...
let wasm: { pulldown_cmark: typeof pulldown_cmark };
...
watch(inputText, (newVal, oldVal) => {
  parsedText.value = wasm?.pulldown_cmark(newVal);
});
</script>

<template>
  <div>
    <input v-model="previewOnly" id="pre" type="checkbox" />
    <label for="pre">preview only</label>
    <div class="cmark">
      <textarea
        v-if="!previewOnly"
        v-model="inputText"
        class="cmark__textarea"
      ></textarea>
      <div v-html="parsedText" class="cmark__markdown"></div>
    </div>
  </div>
</template>
